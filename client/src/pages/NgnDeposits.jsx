...
export default function NgnDeposits({ hideHeader = false }) {
  ...
  const load = useCallback(() => {
    ...
    api.get('/ngn-deposits', { params })
      .then((res) => {
        setDocs(res.data.docs);
        setTotal(res.data.total);
        setTotalPages(res.data.totalPages);
      })
      .catch(() => setError('Failed to load NGN deposits'))
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6">
          ...
          {(search || dateFrom || dateTo) && (
            <button
              onClick={() => { setSearch(''); setDateFrom(''); setDateTo(''); if (searchRef.current) searchRef.current.value = ''; }}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-vs-border bg-vs-elevated/40">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-vs-text-3">Depositor Name</th>
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-vs-text-3">Depositor Acct</th>
                ...
                <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-vs-lime">NGN Deposited</th>
                <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-vs-purple">USD Credited</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="text-center py-12 text-vs-text-3 text-sm">
                    No deposits found{search || dateFrom || dateTo ? ' for this filter' : ''}.
                  </td>
                </tr>
              ) : (
                docs.map((d) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {d.username
                          ...
                          : <span className="text-vs-text-3 text-xs italic">unmatched</span>}
                      </td>
                      <td className="px-4 py-3 font-medium text-vs-text text-xs">{d.depositorName || '—'}</td>
                      <td className="px-4 py-3 font-mono text-xs text-vs-text-2">{d.depositorAccountNumber || '—'}</td>
                      <td className="px-4 py-3 font-mono text-xs text-vs-text-2">{d.platformAccountNumber || '—'}</td>
                      <td className="px-4 py-3 text-xs text-vs-text-2 max-w-[140px] truncate" title={d.platformAccountName}>{d.platformAccountName || '—'}</td>
                      <td className="px-4 py-3 text-right font-semibold text-vs-lime whitespace-nowrap">{fmtNGN(d.amountNGN)}</td>
                      <td className="px-4 py-3 text-right font-semibold text-vs-purple whitespace-nowrap">
                        {d.walletCreditUSD != null ? fmtUSD(d.walletCreditUSD) : <span className="text-vs-text-3 text-xs">—</span>}
                      </td>
                        ...
                        <td colSpan={9} className="px-6 py-4">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-3 text-xs">
                            <div>
                              ...
                              <p className="font-mono text-vs-text break-all">{d.paymentReference || '—'}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              <p className="text-vs-text-3 mb-0.5">Wallet Tx Status</p>
                              <StatusBadge status={d.txStatus} />
                            </div>
                            <div>
                            ...
                            <div>
                              <p className="text-vs-text-3 mb-0.5">Net Received</p>
                              <p className="font-semibold text-vs-lime">
                                {d.amountNGN != null && d.fees != null && d.vat != null
                                  ? fmtNGN(d.amountNGN - (d.fees ?? 0) - (d.vat ?? 0) - (d.stampDuty ?? 0))
                                  : '—'}
                              </p>
                            </div>
                          </div>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-vs-border bg-vs-elevated/20">
            ...
            <div className="flex gap-1">
              <button
                onClick={() => setPage(1)}
                disabled={page === 1}
                className="px-2 py-1 text-xs rounded border border-vs-border text-vs-text-3 disabled:opacity-30 hover:bg-vs-elevated transition-colors"
              >«</button>
              <button
